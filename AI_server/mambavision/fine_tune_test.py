...
class CombinedTripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if idx < len(self.dataset1):
            ...
            return self.dataset2[idx - len(self.dataset1)]  # Get item from the second dataset
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply standard transformation (resize, normalization, etc.)
        if self.transform:
...
def load_images_from_folders(base_folder):
    ...
    for root, _, files in os.walk(base_folder):
        files = [file for file in files if file.lower().endswith(".bmp")]  # Filter only .bmp files
...
triplet_argumentation_dataset = TripletDataset(image_paths, labels, augmentation=augmentation)
# Split the dataset into training and validation sets (e.g., 80% training, 20% validation)
combined_dataset = CombinedTripletDataset(triplet_default_dataset, triplet_argumentation_dataset)
...
def train(model, train_loader, val_loader, criterion, optimizer, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        epoch_iterator = tqdm(train_loader, desc=f"Epoch [{epoch+1}/{epochs}]", unit="batch")
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write(f"Epoch [{epoch+1}/{epochs}], Training Loss: {avg_loss}, Validation Loss: {val_loss}\n")

# Run the training process with progress bars and validation
train(model, train_loader, val_loader, triplet_loss, optimizer, epochs=5)
